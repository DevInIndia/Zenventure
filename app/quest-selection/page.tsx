...
import { getAvailableQuests, addPremadeQuests } from "@/lib/firestore";
...
export default function QuestSelectionPage() {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      // Add selected quests to user's premade quests
      await addPremadeQuests(selected);

      // Redirect to dashboard
      router.push("/dashboard");
    } catch (error) {
      console.error("Error saving selected quests:", error);
        ...
        <Card className="pixel-card bg-[#352f44] text-[#f0ece2] border-[#dbd8e3]">
          ...
          <CardFooter className="border-t-4 border-[#5c5470] pt-4 flex justify-between items-center">
            ...
            <Button
              ...
              disabled={selectedQuests.length === 0 || isSubmitting}
